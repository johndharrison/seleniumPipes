...
#' @rdname addCookie
addCookie <- function(remDr, name, value, path = NULL, domain = NULL,
  ...
  jsonBody <- toJSON(list(cookie = cookie), null = "null",
                     auto_unbox = TRUE)
  pathTemplate <-
    whisker.render("/session/{{sessionId}}/cookie", data = obj)
...
deleteCookie <- function(remDr, name = NULL, ...){
  ...
  obj$name <- name
  pathTemplate <-
    whisker.render("/session/{{sessionId}}/cookie/{{name}}", data = obj)
  pathURL <- remDr[['remServAdd']]
  pathURL[['path']] <- paste0(pathURL[['path']], pathTemplate)
  res <- queryDriver(verb = DELETE, url = build_url(pathURL),
                     source = "deleteCookie",
