...
elementClear <- function(webElem, ...){
  ...
  res <- queryDriver(verb = POST, url = build_url(pathURL), source = "elementClear", drvID = webElem$drvID, json = jsonBody,...)
  invisible(webElem)
}
...
elementClick <- function(webElem, ...){
  ...
  pathURL <- webElem[['remDr']][['remServAdd']]
  pathURL[['path']] <- paste0(pathURL[['path']], pathTemplate)
  res <- queryDriver(verb = POST, url = build_url(pathURL), source = "elementClick", drvID = webElem$drvID, json = jsonBody,...)
  invisible(webElem)
}
...
elementSendKeys <- function(webElem, ...){
  ...
  pathURL <- webElem[['remDr']][['remServAdd']]
  pathURL[['path']] <- paste0(pathURL[['path']], pathTemplate)
  res <- queryDriver(verb = POST, url = build_url(pathURL), source = "elementSendKeys", drvID = webElem$drvID, json = jsonBody,...)
  invisible(webElem)
}
